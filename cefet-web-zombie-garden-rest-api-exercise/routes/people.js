...
router.put('/adiciona/', function(req, res) {
  db.query('INSERT INTO person VALUES (NULL, ' + db.escape(req.body.name) + ', 1, NULL)',
    function(err, result) {
      if (result.affectedRows !== 1) {
        req.flash('error', 'Nao ha pessoa para ser adicionada');
      } else {
...
router.delete('/:name', function(req, res) {
  db.query('DELETE FROM person WHERE id = ' + db.escape(req.params.name),
    function(err, result) {
	console.log(err);
      if (result.affectedRows !== 1) {
        req.flash('error', 'Nao ha pessoa para ser excluida');
      } else {
